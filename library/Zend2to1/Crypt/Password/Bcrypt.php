 ...
 * @package    Zend_Crypt
 */
class Zend2to1_Crypt_Password_Bcrypt implements Zend2to1_Crypt_Password_PasswordInterface
{
    ...
    public function __construct($options = array())
    {
        if (!empty($options)) {
            if ($options instanceof Traversable) {
                $options = Zend2to1_Stdlib_ArrayUtils::iteratorToArray($options);
            } elseif (!is_array($options)) {
                throw new Zend2to1_Crypt_Password_Exception_InvalidArgumentException(
    ...
    {
        if (empty($this->salt)) {
            $salt = Zend2to1_Math_Rand::getBytes(self::MIN_SALT_SIZE);
        } else {
            $salt = $this->salt;
        }
        ...
        if (version_compare(PHP_VERSION, '5.3.7') >= 0) {
            ...
            if (preg_match('/[\x80-\xFF]/', $password)) {
                throw new Zend2to1_Crypt_Password_Exception_RuntimeException(
        ...
        if (strlen($hash) <= 13) {
            throw new Zend2to1_Crypt_Password_Exception_RuntimeException('Error during the bcrypt generation');
        }
        return $hash;
    }
